...
do
  ...
  if [ $(ls *.m2ts | wc -l) -gt 0 ] ; then
    ...
    /home/chinachu/chinachu/usr/bin/ffmpeg -v 0 -re -i $(echo $targetFileName) -ss 2 -threads auto -c:v libx264 -c:a aac -s 1440x1080 -filter:v yadif -b:v 6M -b:a 192k -profile:v baseline -preset ultrafast -tune fastdecode,zerolatency -movflags frag_keyframe+empty_moov+faststart+default_base_moof -y -f mp4 $(echo $newMp4FileName)
    if [ $? = 0 ]; then
      echo "正常にエンコード終了"
      counter=$(( counter + 1 ))
